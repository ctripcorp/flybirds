...
from flybird.core.config_manage import FlowBehave
from flybird.report import json_format_deal
from flybird.report.rerun_params import get_rerun_params
from flybird.utils import file_helper
from flybird.utils import flybird_log as log
from flybird.utils import language_helper as lge
from flybird.utils import uuid_helper
            ...
            try:
                ...
                if isinstance(report_json, list):
                    for feature in report_json:
                        ...
                        rerun_feature_name = (
                            f"flybirdARFeature" f"{rerun_feature_index}"
                        )
...
def set_rerun_info(user_data, gr):
    ...
    if (
        "flybirdAutoRerun" in user_data.keys()
        and user_data["flybirdAutoRerun"] == "Yes"
    ):
        log.info("this is rerun case execute")
        rerun_info_array = user_data["flybirdAutoRerunInfo"].split(",")
        if len(rerun_info_array) > 0:
            for item_rerun_info in rerun_info_array:
                ...
                if (
                    "flybirdAutoRerunInfo" in user_data.keys()
