...
import flybird.template as template
...
def create_demo():
    ...
    """
    typer.secho(
        "Welcome to flybird cli. Please enter any information to continue.",
    ...
    if test_platform is not None and test_platform.strip().lower() == 'ios':
        ...
        if is_bundle:
            ...
            typer.secho(
                ...
                " the project's"
                " flybird_config.json file.", fg=typer.colors.YELLOW)
...
def copy_from_template(progress, project_name, test_platform, device_id=None,
    ...
    if test_platform is not None:
        replace_file_content(
            os.path.join(target_path, "config/flybird_config.json"),
            "platform",
            test_platform,
        )
    progress.update(100)
